    ...
    <TouchableOpacity style={{borderBottomColor:'#eee',borderBottomWidth:1,padding:15}} onPress={() => this.props.navigation.navigate('Products', {id: item._id})}>
        <View style={{marginBottom:15}}>
                ...
                <View style={{flexBasis:250}}>
                    ...
                    {item.off != '0' &&
                        <Text style={{fontFamily:'IRANSansMobile',textAlign:'center',marginTop:15,fontSize:14,color:'#752f2f',textDecorationLine:'line-through'}}>{this.ConvertNumToFarsi(item.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","))} تومان</Text>

                      }
                    <Text style={{fontFamily:'IRANSansMobile',textAlign:'center',marginTop:15,fontSize:16,color:'#752f2f'}}> {this.ConvertNumToFarsi((item.price - ((item.price * item.off)/100)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","))} تومان </Text>
               </View>
                <View   >
